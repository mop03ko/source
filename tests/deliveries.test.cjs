...
(async()=>{
 ...
 user={userId:'c',email:COURIER,displayName:'Идэрчулуун'};
 [status,d]=await get();assert.equal(d.count,3);assert.ok(d.items.some(i=>i.id===legacy));
 // Хүргэгчийн тайлан зөвхөн өөрийн мөрөөр хязгаарлагдана.
 [status,d]=await get('?report=1');assert.equal(status,200);assert.equal(d.total,3);
 ...
 assert.equal(ider.total,3);assert.equal(ider.done,2);assert.equal(ider.pending,1);assert.equal(ider.active_days,3);
 assert.equal(ider.last_day,'2026-09-11');
 ...
 assert.equal(d.byMonth.find(m=>m.month==='2026-09').total,2);
 assert.equal(d.byChannel.find(c=>c.channel==='Зөгий').total,1);
 ...
 assert.equal(d.total,2);assert.equal(d.byCourier.length,1);
 assert.equal((await get('?report=1&rfrom=2026'))[0],400);
 console.log('PASS: delivery journal role isolation (marketing/IT blocked), courier self-scoping by email and legacy name, courier status-only permissions, manager CRUD with optimistic locking, date/status/courier validation, list filters, and per-courier dashboard aggregation (performance, months, channels, kinds).');
