...
export function SelectControl({children,value,defaultValue,className,style,onChange,ref,...props}:React.ComponentProps<'select'>){
 const field=useField(),generatedId=React.useId(),id=props.id??generatedId;
 // rc-select points an empty search at `${id}_list_-1`; expose that disabled
 // option through its supported empty-content slot without adding a form value.
