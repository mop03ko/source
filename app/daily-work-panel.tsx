...
import {dateLabel,roles,itStages,marketingStages,deliveryStatuses,isIsolatedRole,canManageSchedule,type Member} from '@/lib/crm';
type Daily={day:string;staffing?:{working:number};shift:{assignment:string;note:string}[];items:{id:string;title:string;status:string;due_at:string;context:string}[];count:number;summary:{due:number;overdue:number;unscheduled:number}|null};
export default function DailyWorkPanel({me,refresh,day,onNavigate,onOpen}:{me:Member;refresh:number;day:string;onNavigate:(view:string)=>void;onOpen:(targetView:string,id:string)=>void}){
 ...
 const management=canManageSchedule(me.role);
 ...
 return <section className="panel daily-work" aria-label="Эрхэд тохирсон өнөөдрийн ажил"><div className="section-heading"><div><div className="eyebrow">{roles[me.role]} · {day}</div><h2>{isolated||me.role==='agent'?'Миний өнөөдрийн ажил':'Өнөөдөр ажиллаж буй ажилтнууд'}</h2><p className="muted">{management?'Өнөөдрийн ажлын хуваарийн дагуу ажиллах ажилтны тоо.':description[me.role]}</p></div><Button variant="outline" onClick={()=>onNavigate('schedule')}>Баг ба хуваарь</Button></div>
  <AsyncStatus error={result.error} loading={result.loading} retry={result.retry}/>
  {d&&!result.loading&&!result.error&&<>{management?<Card size="small" className="daily-staffing"><Statistic title="Өнөөдөр ажиллах нийт ажилтан" value={d.staffing?.working??0} suffix="хүн"/><p className="muted">Амралт, чөлөө болон томилгоогүй ажилтныг оруулаагүй. Ирцийн бүртгэл биш, хуваарьт үндэслэсэн тоо.</p></Card>:<div className="daily-shift"><strong>Миний өнөөдрийн хуваарь:</strong> {d.shift.length?d.shift.map((s,i)=><Tag key={i} title={s.note}>{s.assignment}</Tag>):<span className="muted">Хуваарь бүртгээгүй</span>}</div>}
