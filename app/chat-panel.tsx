...
import {Input as AntInput,Segmented,Empty,Spin} from 'antd';
...
export default function ChatPanel({me,members,onRead}:{me:Member;members:Member[];onRead:()=>void}){
 ...
 const [search,setSearch]=useState(''),[listFilter,setListFilter]=useState('all'),[threadLoading,setThreadLoading]=useState(false),[hasOlder,setHasOlder]=useState(false),[olderLoading,setOlderLoading]=useState(false);
 const sending=useRef(false),historyExhausted=useRef(false),threadSequence=useRef(0),listSequence=useRef(0),atBottom=useRef(true);
 const lastReadThrough=useRef<Record<string,string>>({});
 const now=useClock();
 ...
 const onMessagesScroll=()=>{const el=messagesRef.current;if(!el)return;const bottom=el.scrollHeight-el.scrollTop-el.clientHeight<80;atBottom.current=bottom;setStickToBottom(bottom);};
 ...
 const pickImage=async(e:React.ChangeEvent<HTMLInputElement>)=>{const targetPeer=peer;const file=e.target.files?.[0];e.target.value='';if(!file)return;setImageError('');if(!['image/png','image/jpeg','image/webp','image/gif'].includes(file.type)){setImageError('Зөвхөн зураг сонгоно уу.');return;}if(file.size>MAX_IMAGE){setImageError('Зургийн хэмжээ 5MB-аас бага байна.');return;}try{const data=await new Promise<string>((resolve,reject)=>{const reader=new FileReader();reader.onerror=()=>reject(new Error('Файл уншиж чадсангүй.'));reader.onload=()=>resolve(reader.result as string);reader.readAsDataURL(file);});if(selectedPeer.current===targetPeer)setImage(data);}catch(e){setImageError((e as Error).message);}};
 const loadConversations=useCallback(async()=>{const sequence=++listSequence.current;try{const [c,s]=await Promise.all([request() as Promise<{items:Conversation[]}>,request(undefined,'?summary=1') as Promise<Summary>]);if(sequence!==listSequence.current)return;channelIdsRef.current=new Set(s.channels.map(c=>c.channel));setConversations(c.items);setSummary(s);if(!selectedPeer.current)setError('');}catch(e){if(sequence===listSequence.current)setError((e as Error).message);}},[]);
 ...
 const loadThread=useCallback(async(p:string)=>{const sequence=++threadSequence.current;try{const channel=channelIdsRef.current.has(p);const d=await request(undefined,channel?'?team=1&channel='+encodeURIComponent(p):'?peer='+encodeURIComponent(p)) as {items:Msg[];reads?:TeamRead[];members?:ChannelMember[]};if(selectedPeer.current!==p||sequence!==threadSequence.current)return null;setThread(previous=>{const earliest=d.items[0];const older=earliest?previous.filter(m=>!m.id.startsWith('tmp-')&&(m.created_at<earliest.created_at||(m.created_at===earliest.created_at&&m.id<earliest.id))):[];return [...older,...d.items];});setHasOlder(!historyExhausted.current&&d.items.length===200);setTeamReads(d.reads||[]);setChannelMembers(d.members||[]);setThreadLoading(false);setError('');return d.items.at(-1)?.created_at||null;}catch(e){if(selectedPeer.current===p&&sequence===threadSequence.current){setError((e as Error).message);setThreadLoading(false);}return null;}},[]);
 const loadOlder=async()=>{if(olderLoading||!thread.length)return;const p=peer;setOlderLoading(true);setStickToBottom(false);atBottom.current=false;const oldest=thread.find(m=>!m.id.startsWith('tmp-'));try{const d=await request(undefined,(isChannel(p)?'?team=1&channel=':'?peer=')+encodeURIComponent(p)+'&before='+encodeURIComponent(oldest!.id)) as {items:Msg[]};if(selectedPeer.current!==p)return;const element=messagesRef.current,previousHeight=element?.scrollHeight||0;setThread(current=>{const ids=new Set(current.map(m=>m.id));return [...d.items.filter(m=>!ids.has(m.id)),...current];});historyExhausted.current=d.items.length<200;setHasOlder(!historyExhausted.current);requestAnimationFrame(()=>{if(element&&selectedPeer.current===p)element.scrollTop+=element.scrollHeight-previousHeight;});}catch(e){if(selectedPeer.current===p)setError((e as Error).message);}finally{if(selectedPeer.current===p)setOlderLoading(false);}};
 ...
 // Refresh without overlapping polls. Mark only the loaded snapshot read while the viewer is at the bottom.
 useEffect(()=>{if(!peer)return;let stopped=false,running=false;const tick=async()=>{if(stopped||running||sending.current||document.visibilityState!=='visible')return;running=true;try{const through=await loadThread(peer);if(stopped||selectedPeer.current!==peer)return;if(through&&atBottom.current&&lastReadThrough.current[peer]!==through){await request(channelIdsRef.current.has(peer)?{action:'read_team',channel:peer,through}:{action:'read',peer,through});lastReadThrough.current[peer]=through;if(!stopped){await loadConversations();onRead();}}}catch(e){if(!stopped)setError((e as Error).message);}finally{running=false;}};const onVisible=()=>void tick();void tick();const timer=setInterval(onVisible,5000);document.addEventListener('visibilitychange',onVisible);return()=>{stopped=true;clearInterval(timer);document.removeEventListener('visibilitychange',onVisible);};},[peer,loadThread,loadConversations,onRead]);
 // Илгээх дарангуутаа мессежийг шууд бөмбөлөгт харуулж (optimistic), дараа нь бодит хариугаар баталгаажуулна.
 const submit=async(e:FormEvent)=>{e.preventDefault();const text=body.trim();if(sending.current||threadLoading||(!text&&!image)||!peer)return;sending.current=true;threadSequence.current++;const tempId='tmp-'+crypto.randomUUID();const replying=replyTarget,pendingImage=image;atBottom.current=true;setStickToBottom(true);setThread(t=>[...t,{id:tempId,sender:me.email,body:text,created_at:new Date().toISOString(),reply_to_id:replying?.id||null,reply_to_sender:replying?.sender||null,reply_to_body:replying?.body||null,image:pendingImage}]);setBody('');setReplyTarget(null);setImage(null);setImageError('');setBusy(true);try{await request(isChannel(peer)?{action:'send_team',channel:peer,body:text,image:pendingImage||undefined,replyTo:replying?.id}:{action:'send',peer,body:text,image:pendingImage||undefined,replyTo:replying?.id});await Promise.all([loadThread(peer),loadConversations()]);}catch(e){setThread(t=>t.filter(m=>m.id!==tempId));setError((e as Error).message);setBody(text);setReplyTarget(replying);setImage(pendingImage);}finally{sending.current=false;setBusy(false);}};
 ...
 const visibleItems=listItems.filter(item=>(listFilter!=='unread'||item.unread>0)&&(listFilter!=='groups'||item.kind==='channel')&&(!search||[item.label,item.member?.email||''].some(value=>value.toLocaleLowerCase().includes(search.trim().toLocaleLowerCase()))));
 ...
 <div className="chat-search"><AntInput.Search aria-label="Чат хайх" placeholder="Нэр, и-мэйлээр хайх" allowClear value={search} onChange={e=>setSearch(e.target.value)}/><Segmented block size="small" aria-label="Чатын шүүлтүүр" value={listFilter} onChange={value=>setListFilter(String(value))} options={[{label:'Бүгд',value:'all'},{label:'Уншаагүй',value:'unread'},{label:'Групп',value:'groups'}]}/></div>{!peer&&error&&<div className="error-box" role="alert">{error}<Button onClick={()=>void loadConversations()}>Дахин оролдох</Button></div>}<div className="chat-list-scroll">
 {visibleItems.map(item=>item.kind==='channel'?<button key={'c-'+item.id} className={'chat-peer chat-peer-pinned'+(peer===item.id?' active':'')+(item.mentioned?' chat-peer-mentioned':'')} aria-pressed={peer===item.id} disabled={busy} onClick={()=>selectPeer(item.id)}><span className="chat-avatar chat-avatar-team"><Users size={16}/></span><span className="chat-peer-info"><strong>{item.label}<Pin size={11} className="pin-icon"/></strong>{item.last&&<small>{item.last.sender===me.email?'Та: ':''}{previewText(item.last)}</small>}</span>{item.mentioned&&<b className="chat-unread chat-mention-badge">@</b>}{!item.mentioned&&!!item.unread&&<b className="chat-unread">{item.unread}</b>}</button>:<button key={'p-'+item.id} className={'chat-peer'+(peer===item.id?' active':'')} aria-pressed={peer===item.id} disabled={busy} onClick={()=>selectPeer(item.id)}><span className="chat-avatar">{item.member!.avatar?<AvatarImage width={192} height={192} unoptimized src={item.member!.avatar} alt=""/>:item.member!.name.slice(0,1)}<i className={'chat-status'+(online(item.id)?' online':'')}/></span><span className="chat-peer-info"><strong>{item.label}</strong>{item.last&&<small>{item.last.mine?'Та: ':''}{previewText(item.last)}</small>}</span>{!!item.unread&&<b className="chat-unread">{item.unread}</b>}</button>)}
 {!loading&&!visibleItems.length&&<Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description={search?'Хайлттай тохирох чат алга.':listFilter==='unread'?'Уншаагүй зурвас алга.':'Чат алга.'}/>}</div></aside>
 ...
 <div className="chat-thread-head"><Button className="chat-back" variant="ghost" size="icon" disabled={busy} aria-label="Чатын жагсаалт руу буцах" onClick={()=>selectPeer('')}><ArrowLeft size={18}/></Button>{!peerIsChannel&&<span className="chat-avatar">{avatarOf(peer)?<AvatarImage width={192} height={192} unoptimized src={avatarOf(peer)!} alt=""/>:name(peer).slice(0,1)}<i className={'chat-status'+(online(peer)?' online':'')}/></span>}<span><strong>{name(peer)}</strong>{peerIsChannel?<small>{channelMembers.length+1} гишүүн · @ нэрээр дурдах</small>:<small>{online(peer)?'Онлайн':'Офлайн'}</small>}</span><ChatSearch key={peer} peer={peer} channel={peerIsChannel} name={name}/></div>
 {error&&<div role="alert" className="error-box">{error}<Button type="button" onClick={()=>void loadThread(peer)}>Дахин ачаалах</Button></div>}
 <div className="chat-messages" ref={messagesRef} onScroll={onMessagesScroll} aria-label="Чатын зурвасууд" aria-busy={threadLoading}>{hasOlder&&<Button type="button" className="chat-load-older" disabled={olderLoading} onClick={()=>void loadOlder()}>{olderLoading?'Ачаалж байна…':'Өмнөх зурвасууд'}</Button>}{threadLoading&&<div className="chat-loading"><Spin/><span>Зурвас ачаалж байна…</span></div>}{!threadLoading&&!error&&!thread.length&&<p className="muted chat-empty-list">Мессеж алга. Эхний мессежээ бичээрэй.</p>}{thread.map((msg,i)=>{const isLast=i===thread.length-1,mine=msg.sender===me.email,mentionsMe=!mine&&(msg.mentions_all||!!msg.mentions?.includes(me.email));return <div key={msg.id} className={'chat-bubble'+(mine?' mine':'')+(mentionsMe?' mentioned':'')}>
 ...
 {!!msg.reactions?.length&&<div className="chat-reactions">{msg.reactions.map(r=><button type="button" key={r.emoji} className={'reaction-pill'+(r.mine?' mine':'')} aria-pressed={r.mine} onClick={()=>react(msg,r.emoji)} title={r.actors.length?r.actors.map(name).join(', '):undefined}>{r.emoji} {r.count}</button>)}</div>}
 <time>{dateLabel(msg.created_at)}</time>
 <div className="bubble-actions" inert={msg.id.startsWith('tmp-')}><ReactionPicker onPick={e=>react(msg,e)}/><button type="button" className="bubble-action" aria-label="Хариулах" onClick={()=>setReplyTarget({id:msg.id,sender:msg.sender,body:msg.body})}><Reply size={13}/></button></div>
 {isLast&&mine&&!peerIsChannel&&<small className="seen-tag">{msg.id.startsWith('tmp-')?'Илгээж байна…':msg.read_at?'Үзсэн':'Илгээсэн'}</small>}
 ...
 <form className="chat-composer" onSubmit={submit}><fieldset disabled={busy} className="chat-composer-tools"><EmojiPicker onPick={e=>setBody(b=>(b+e).slice(0,2000))}/><Button type="button" variant="ghost" size="icon" aria-label="Зураг хавсаргах" onClick={()=>fileInputRef.current?.click()}><ImagePlus size={18}/></Button><input ref={fileInputRef} type="file" accept="image/png,image/jpeg,image/webp,image/gif" hidden onChange={pickImage}/></fieldset><AntInput.TextArea ref={bodyRef} disabled={busy} autoSize={{minRows:1,maxRows:5}} onKeyDown={e=>{if(e.key==='Enter'&&!e.shiftKey&&!e.nativeEvent.isComposing){e.preventDefault();e.currentTarget.form?.requestSubmit();}}} aria-label="Мессеж бичих" value={body} maxLength={2000} placeholder="Зурвас бичих…" onChange={e=>onBodyChange(e.target.value)}/><Button className="primary" type="submit" aria-label="Мессеж илгээх" disabled={busy||threadLoading||(!body.trim()&&!image)}>{busy?<Loader2 size={16} className="spin"/>:<Send size={16}/>}</Button></form><div className="chat-composer-hint"><span>Enter — илгээх · Shift+Enter — шинэ мөр</span><span>{body.length}/2000</span></div>
 </div>
 </>}</section>
