...
type ChannelSummary={channel:string;label:string;unread:number;last:{sender:string;body:string;created_at:string}|null};
...
export default function ChatPanel({me,members,onRead}:{me:Member;members:Member[];onRead:()=>void}){
 ...
 {(summary?.channels||[]).map(c=><button key={c.channel} className={'chat-peer'+(peer===c.channel?' active':'')} onClick={()=>setPeer(c.channel)}><span className="chat-avatar chat-avatar-team"><Users size={16}/></span><span className="chat-peer-info"><strong>{c.label}</strong>{c.last&&<small>{c.last.sender===me.email?'Та: ':''}{c.last.body}</small>}</span>{!!c.unread&&<b className="chat-unread">{c.unread}</b>}</button>)}
 ...
 <div className="chat-thread-head">{!peerIsChannel&&<span className="chat-avatar">{avatarOf(peer)?<img src={avatarOf(peer)!} alt=""/>:name(peer).slice(0,1)}<i className={'chat-status'+(online(peer)?' online':'')}/></span>}<span><strong>{name(peer)}</strong>{!peerIsChannel&&<small>{online(peer)?'Онлайн':'Идэвхгүй'}</small>}</span></div>
 ...
 {peerIsChannel&&!mine&&<small><span className="chat-avatar chat-avatar-tiny">{avatarOf(msg.sender)?<img src={avatarOf(msg.sender)!} alt=""/>:name(msg.sender).slice(0,1)}</span>{name(msg.sender)}</small>}
