...
function RuleRow({rule,onChanged}:{rule:Rule;onChanged:()=>Promise<void>}){
 ...
 <label className="field"><span>{label} · Илгээх мессеж</span><textarea disabled={busy} rows={3} maxLength={600} value={message} onChange={e=>setMessage(e.target.value)}/></label>
...
export default function SmsRulesPanel(){
 ...
 {!!available.length&&<form className="form-stack sms-rule-add" onSubmit={addRule}><h2>Шинэ дүрэм</h2><label className="field"><span>Төлөв</span><select value={selected} onChange={e=>setNewStatus(e.target.value)}>{available.map(([k,v])=><option key={k} value={k}>{v}</option>)}</select></label><label className="field"><span>Мессеж</span><textarea required rows={3} maxLength={600} value={newMessage} onChange={e=>setNewMessage(e.target.value)}/></label><Button className="primary" disabled={addBusy||!newMessage.trim()}><Plus size={16}/>Дүрэм нэмэх</Button></form>}</>}
 </section>;
}
