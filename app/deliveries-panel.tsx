...
import {Truck,Search,Plus,List,ChartNoAxesCombined,CheckCircle2,Clock,CircleX,ArrowUpRight,Loader2,Package} from 'lucide-react';
...
type Stats={total:number;done:number;pending:number;failed:number};
...
type Report={total:number;byCourier:CourierRow[];byMonth:{month:string;total:number;done:number}[];byChannel:{channel:string;total:number}[];byKind:{kind:string;total:number}[];byStatus:{status:string;total:number}[]};
type List={items:Delivery[];count:number;stats:Stats;couriers:{name:string;total:number}[];channels:{name:string}[]};
...
function DeliveryForm({row,members,legacy,busy,onSubmit}:{row?:Delivery;members:Member[];legacy:string[];busy:boolean;onSubmit:(d:unknown)=>unknown}){
 ...
 <Field label="Барааны мэдээлэл"><Input name="item_info" maxLength={400} defaultValue={row?.item_info} placeholder="Жишээ: Dyson V15s, IP 17 PM"/></Field>
...
export default function DeliveriesPanel({me,members}:{me:Member;members:Member[]}){
 ...
 const detail=useRemote<{delivery:Delivery}>(detailId?'/api/deliveries?id='+encodeURIComponent(detailId)+'&revision='+revision:null);
 ...
 <div className="next-box"><Truck size={18}/><div><strong>{d.item_info||'Барааны мэдээлэл бүртгээгүй'}</strong><p>{d.customer_phone||'утасгүй'} · {d.address||'хаяггүй'}</p><p className="muted">Бүртгэсэн: {d.entered_by_name||d.created_by} · {requestDateLabel(d.created_at)}</p></div></div>
