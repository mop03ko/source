...
type List={items:Row[];groups?:Group[];count:number;summary:Record<string,number>;truncated?:boolean};
...
export default function InventoryPanel({me,members}:{me:Member;members:Member[]}){
 const [mode,setMode]=useState<Mode>('items'),[q,setQ]=useState(''),[warehouse,setWarehouse]=useState(''),[brand,setBrand]=useState(''),[supplier,setSupplier]=useState(''),[group,setGroup]=useState(''),[stock,setStock]=useState(''),[status,setStatus]=useState(''),[page,setPage]=useState(1),[revision,setRevision]=useState(0);
 ...
 params.set('brand',brand);params.set('supplier',supplier);
 ...
 const exportCsv=async(selectedOnly=false)=>{
  setExporting(true);try{
   ...
   if(grouped){
    ...
    headers=['Код','Нэр','Брэнд','Нийлүүлэгч','Багтаамж','Өнгө','IMEI',...(mode==='balance'?['Эхний тоо','Эхний өртөг','Орлого тоо','Орлого өртөг','Зарлага тоо','Зарлага өртөг']:[]),'Эцсийн тоо','Эцсийн өртөг','Үндсэн үнэ','Бэлэн үнэ','Өртөг тооцоолсон'];
    values=d.items.map(i=>[i.code,i.name,i.brand,i.supplier,i.capacity,i.color,i.imei,...(mode==='balance'?[i.opening_qty,i.opening_cents/100,i.in_qty,i.in_cents/100,i.out_qty,i.out_cents/100]:[]),i.stock,i.value_cents/100,i.sale_price,i.cash_price??i.sale_price,i.cost_estimated?'Тийм':'Үгүй']);
   }else if(mode==='sales'){
 ...
 return <section className="table-panel inventory-panel">
  ...
  {mode==='counts'?<InventoryCountsPanel me={me} members={members}/>:<>
   ...
   <ResponsiveFilters className="filters inventory-filters" active={[warehouse,brand,supplier,stock,status,...(mode!=='items'?[from,to]:[])].filter(Boolean).length}>
    ...
    {(mode==='items'||mode==='balance')&&<><SelectControl aria-label="Үлдэгдлээр шүүх" value={stock} onChange={e=>setFilter(setStock,e.target.value)}><option value="">Бүх үлдэгдэл</option><option value="positive">Үлдэгдэлтэй</option><option value="empty">Үлдэгдэлгүй</option><option value="low">Доод хэмжээнд хүрсэн</option></SelectControl></>}
    ...
    <Button variant="ghost" size="sm" onClick={()=>{setWarehouse('');setBrand('');setSupplier('');setStock('');setStatus('');setFrom('');setTo('');setQ('');setPage(1);}}>Шүүлтүүр цэвэрлэх</Button>
   </ResponsiveFilters>
   {['items','balance','sales'].includes(mode)&&<div className="inventory-group-control"><Field label="Тайлангийн ангилал"><SelectControl aria-label="Тайлангийн ангилал" value={group} onChange={e=>setFilter(setGroup,e.target.value)}><option value="">Бараа тус бүрээр</option><option value="brand">Брэндээр нэгтгэх</option><option value="supplier">Нийлүүлэгчээр нэгтгэх</option></SelectControl></Field></div>}
  ...
  <Sheet open={!!detailId} onOpenChange={o=>{if(!o)setDetailId('');}}><SheetContent className="detail-sheet inventory-detail"><SheetHeader><SheetTitle>{detail.data?.item.name||'Барааны дэлгэрэнгүй'}</SheetTitle><SheetDescription>Агуулах тус бүрийн үлдэгдэл, өртөг, сүүлийн 100 хөдөлгөөн</SheetDescription></SheetHeader><div className="detail-body"><AsyncStatus error={detail.error} loading={detail.loading} retry={detail.retry}/>{detail.data&&<>
   ...
   <Descriptions bordered size="small" column={1} items={[
    ['Код / SKU',detail.data.item.code],['Брэнд',detail.data.item.brand],['Нийлүүлэгч',detail.data.item.supplier],
