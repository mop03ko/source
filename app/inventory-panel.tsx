'use client';
import {Descriptions,Tag,Checkbox,Segmented} from 'antd';
...
export default function InventoryPanel({me,members}:{me:Member;members:Member[]}){
 ...
 const [catalog,setCatalog]=useState(true),[product,setProduct]=useState<Item|null>(null);
 ...
 return <section className="table-panel inventory-panel">
  ...
  {mode==='counts'?<InventoryCountsPanel me={me} members={members}/>:<>
   ...
   {mode==='items'&&!grouped&&<div className="inventory-catalog-control"><Segmented aria-label="Барааны харагдац" value={catalog?'products':'units'} onChange={v=>{setCatalog(v==='products');setPage(1);}} options={[{value:'products',label:'Бараагаар нэгтгэсэн'},{value:'units',label:'Бүх дугаар'}]}/><p className="muted">Ижил загвар, багтаамж, өнгө нэг бараа. Дотор нь IMEI, баркод, кодоор ялгана.</p></div>}
  ...
  {product&&<InventoryProductDetail key={product.id} id={product.id} initialProduct={product} revision={revision} warehouse={warehouse} onClose={()=>setProduct(null)} onOpenUnit={id=>{setProduct(null);setDetailId(id);}} onAdd={p=>{setProduct(null);setModal({kind:'item',template:{...p,id:'',code:'',barcode:'',imei:null,supplier:p.supplier==='Олон нийлүүлэгч'?'':p.supplier,category:p.category==='Олон ангилал'?'':p.category,min_stock:0}});}}/>}
  <Sheet open={!!detailId} onOpenChange={o=>{if(!o)setDetailId('');}}><SheetContent className="detail-sheet inventory-detail"><SheetHeader><SheetTitle>{detail.data?.item.name||'Барааны дэлгэрэнгүй'}</SheetTitle><SheetDescription>Агуулах тус бүрийн үлдэгдэл, өртөг, сүүлийн 100 хөдөлгөөн</SheetDescription></SheetHeader><div className="detail-body"><AsyncStatus error={detail.error} loading={detail.loading} retry={detail.retry}/>{detail.data&&<>
   <StockTag item={detail.data.item}/>{(detail.data.identifiers?.length||0)>0&&<details className="inventory-unit-history"><summary>Гарсан дугаарын бүртгэл</summary>{detail.data.identifiers?.map(u=><p key={u.id}>IMEI/сериал: {u.serial||'—'} · Баркод: {u.barcode||'—'}</p>)}</details>}
  ...
  <Dialog open={!!modal} onOpenChange={open=>{if(!open&&!busy)setModal(null);}}><DialogContent className="form-dialog inventory-dialog"><DialogHeader><DialogTitle>{modal?.kind==='item'?(modal.item?'Барааны мэдээлэл засах':'Бараа нэмэх'):modal?modalTitle[modal.kind]:''}</DialogTitle><DialogDescription>{modal?.kind==='import'?'Үлдэгдлийг импортлохоос өмнө файл, огноо болон зөрчлийг шалгана.':'Мэдээллээ бөглөөд хадгална уу.'}</DialogDescription></DialogHeader>
   {modal?.kind==='item'&&(!modal.item||canEditItem)&&<ItemForm item={modal.item||modal.template} busy={busy} onSave={data=>save(modal.item?'update_item':'create_item',data,modal.item?.id)}/>}
