 ...
 return <section className="table-panel inventory-panel">
  ...
  {mode==='counts'?<InventoryCountsPanel me={me} members={members}/>:<>
   ...
   <div className="filters inventory-filters">
    ...
    <select aria-label="Агуулахаар шүүх" value={warehouse} onChange={e=>setFilter(setWarehouse,e.target.value)}><option value="">Бүх агуулах</option>{opts.warehouses.map(w=><option key={w.id} value={w.id}>{w.name}</option>)}</select>
    {(mode==='items'||mode==='balance')&&<><select aria-label="Брэндээр шүүх" value={brand} onChange={e=>setFilter(setBrand,e.target.value)}><option value="">Бүх брэнд</option>{opts.brands.map(b=><option key={b.brand}>{b.brand}</option>)}</select><select aria-label="Үлдэгдлээр шүүх" value={stock} onChange={e=>setFilter(setStock,e.target.value)}><option value="">Бүх үлдэгдэл</option><option value="positive">Үлдэгдэлтэй</option><option value="empty">Үлдэгдэлгүй</option><option value="low">Доод хэмжээнд хүрсэн</option></select></>}
    {mode==='purchases'&&<select aria-label="Худалдан авалтын төлөв" value={status} onChange={e=>setFilter(setStatus,e.target.value)}><option value="">Бүх төлөв</option>{Object.entries(purchaseStatuses).map(([k,v])=><option key={k} value={k}>{v}</option>)}</select>}
