...
export default function InventoryPanel({me,members}:{me:Member;members:Member[]}){
 ...
 const stock=query.get(mode+'_stock'),setStock=(v:string)=>query.set(mode+'_stock',v);
 ...
 const content=(mode==='counts'?<InventoryCountsPanel me={me} members={members}/>:<>
   ...
   <ResponsiveFilters className="filters inventory-filters" active={[warehouse,brand,supplier,category,...(['items','balance'].includes(mode)?[stock]:[]),...(mode==='purchases'?[status]:[]),...(mode==='moves'?[movementKind]:[]),...(mode!=='items'?[from,to]:[])].filter(Boolean).length}>
    ...
    <Button variant="ghost" size="sm" onClick={()=>{setWarehouse('');setBrand('');setSupplier('');setCategory('');setStock('');setStatus('');setMovementKind('');query.set('ref','');setFrom(mode==='balance'?today.slice(0,7)+'-01':'');setTo(mode==='balance'?today:'');setQ('');setPage(1);}}>Шүүлтүүр цэвэрлэх</Button>
   </ResponsiveFilters>
   <div className="inventory-filter-tags">{mode==='moves'&&query.get('ref')&&<Tag closable onClose={()=>query.set('ref','')}>Холбогдсон шилжүүлэг</Tag>}{[[warehouse,opts.warehouses.find(w=>w.id===warehouse)?.name||warehouse,setWarehouse],[brand,brand,setBrand],[supplier,supplier,setSupplier],[category,category==='__uncategorized__'?'Ангилаагүй':category,setCategory],...(['items','balance'].includes(mode)?[[stock,({positive:'Үлдэгдэлтэй',empty:'Үлдэгдэлгүй',low:'Доод хэмжээнд',reorder:'Нөхөн татах'} as Record<string,string>)[stock],setStock]]:[])].filter(([value])=>value).map(([value,label,setter])=><Tag key={String(value)+String(label)} closable onClose={()=>setFilter(setter as (v:string)=>void,'')}>{String(label)}</Tag>)}</div>
