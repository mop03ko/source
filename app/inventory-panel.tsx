...
import {Descriptions,Tag,Checkbox,Segmented,Dropdown,Popconfirm,Skeleton} from 'antd';
...
import {canViewInventoryCost,canEditInventoryItem,dateLabel,type Member} from '@/lib/crm';
...
export default function InventoryPanel({me,members}:{me:Member;members:Member[]}){
 ...
 const dimension=(['category','brand','supplier','warehouse'].includes(query.get('balance_breakdown'))?query.get('balance_breakdown'):'category') as BalanceDimension;
 const [balanceView,setBalanceView]=useState<BalanceView>('all');
 const requestedSort=query.get(mode+'_sort',mode==='items'?catalogPrefs.preferences.sort:'name'),sort=!showCost&&requestedSort==='value_desc'?'name':requestedSort,setSort=(v:string)=>{query.set(mode+'_sort',v);if(mode==='items')catalogPrefs.update({sort:v});};
 ...
 if(mode==='balance')params.set('breakdown',dimension);
 ...
 const exportCsv=async(selectedOnly=false)=>{
  setExporting(true);try{
   ...
   if(!showCost){const visible=headers.map((h,i)=>costLabel(h)?-1:i).filter(i=>i>=0);headers=visible.map(i=>headers[i]);values=values.map(row=>visible.map(i=>row[i]));}
 ...
 const content=(mode==='sheet_sync'?<InventorySheetPanel onSynced={()=>setRevision(v=>v+1)}/>:mode==='counts'?<InventoryCountsPanel me={me} members={members}/>:<>
  {mode==='sales'&&['admin','director','manager','agent','operator'].includes(me.role)&&<DirectSaleRequests me={me} revision={revision} onChange={()=>setRevision(v=>v+1)}/>}
   ...
   <ResponsiveFilters className="filters inventory-filters" active={[warehouse,brand,supplier,category,...(['items','balance'].includes(mode)?[stock]:[]),...(mode==='purchases'?[status]:[]),...(mode==='moves'?[movementKind]:[]),...(mode!=='items'?[from,to]:[])].filter(Boolean).length}>
    ...
    <SelectControl aria-label="Брэндээр шүүх" value={brand} onChange={e=>setFilter(setBrand,e.target.value)}><option value="">Бүх брэнд</option>{brand==='__unregistered__'&&<option value="__unregistered__">Брэнд тодорхойгүй</option>}{opts.brands.map(b=><option key={b.brand}>{b.brand}</option>)}</SelectControl><SelectControl aria-label="Нийлүүлэгчээр шүүх" value={supplier} onChange={e=>setFilter(setSupplier,e.target.value)}><option value="">Бүх нийлүүлэгч</option>{supplier==='__unregistered__'&&<option value="__unregistered__">Нийлүүлэгч тодорхойгүй</option>}{(opts.suppliers||[]).map(s=><option key={s.supplier}>{s.supplier}</option>)}</SelectControl>
    {(mode==='items'||mode==='balance')&&<><SelectControl aria-label="Үлдэгдлээр шүүх" value={stock} onChange={e=>setFilter(setStock,e.target.value)}><option value="nonzero">0 үлдэгдлийг нуух</option><option value="">Бүх үлдэгдэл</option><option value="positive">Үлдэгдэлтэй</option><option value="negative">Сөрөг үлдэгдэл</option><option value="empty">Үлдэгдэлгүй</option><option value="low">Доод хэмжээнд хүрсэн</option><option value="reorder">Нөхөн татах</option></SelectControl></>}
   ...
   <SavedViews key={me.email+me.role} scope={me.email+":"+me.role+":inventory"}/><div className="inventory-filter-tags">{mode==='moves'&&query.get('ref')&&<Tag closable onClose={()=>query.set('ref','')}>Холбогдсон шилжүүлэг</Tag>}{[[warehouse,opts.warehouses.find(w=>w.id===warehouse)?.name||warehouse,setWarehouse],[brand,brand==='__unregistered__'?'Брэнд тодорхойгүй':brand,setBrand],[supplier,supplier==='__unregistered__'?'Нийлүүлэгч тодорхойгүй':supplier,setSupplier],[category,category==='__uncategorized__'?'Ангилаагүй':category,setCategory],...(['items','balance'].includes(mode)?[[stock,({nonzero:'0 үлдэгдлийг нуух',positive:'Үлдэгдэлтэй',empty:'0 буюу сөрөг үлдэгдэл',negative:'Сөрөг үлдэгдэл',low:'Доод хэмжээнд',reorder:'Нөхөн татах'} as Record<string,string>)[stock],setStock]]:[])].filter(([value])=>value).map(([value,label,setter])=><Tag key={String(value)+String(label)} closable onClose={()=>setFilter(setter as (v:string)=>void,'')}>{String(label)}</Tag>)}</div>
   ...
   {mode==='balance'&&!list.loading&&!list.error&&summary&&list.data?.report&&<InventoryBalanceReport summary={summary} categories={list.data.report.breakdown||list.data.report.categories} dimension={dimension} onDimension={v=>query.set('balance_breakdown',v)} from={from||today.slice(0,7)+'-01'} to={to||today} warehouse={opts.warehouses.find(w=>w.id===warehouse)?.name||'Бүх агуулах'} view={balanceView} onView={setBalanceView} onStock={value=>setFilter(setStock,value)} onCategory={value=>{({category:setCategory,brand:setBrand,supplier:setSupplier,warehouse:setWarehouse})[dimension](value);setGroup('');setBalanceView('list');setPage(1);}}/>}
   {['items','balance','sales'].includes(mode)&&(mode!=='balance'||balanceView!=='charts')&&<details className="inventory-group-control" open={mode==='balance'?true:undefined}><summary>Харагдац · {grouped?({brand:'Брэнд',category:'Ангилал',supplier:'Нийлүүлэгч'} as Record<string,string>)[grouped]:'Бараа тус бүрээр'}</summary><Field label="Тайлангийн ангилал"><SelectControl aria-label="Тайлангийн ангилал" value={group} onChange={e=>setFilter(setGroup,e.target.value)}><option value="">Бараа тус бүрээр</option><option value="category">Ангиллаар нэгтгэх</option><option value="brand">Брэндээр нэгтгэх</option><option value="supplier">Нийлүүлэгчээр нэгтгэх</option></SelectControl></Field>{mode==='items'&&!grouped&&<Field label="Жагсаалт эрэмбэлэх"><SelectControl aria-label="Бараа эрэмбэлэх" value={sort} onChange={e=>setFilter(setSort,e.target.value)}><option value="name">Нэрээр</option><option value="name_desc">Нэр буурахаар</option><option value="stock_asc">Үлдэгдэл багаас</option><option value="stock_desc">Үлдэгдэл ихээс</option><option value="price_asc">Үнэ багаас</option><option value="price_desc">Үнэ ихээс</option>{showCost&&<option value="value_desc">Өртөг ихээс</option>}</SelectControl></Field>}</details>}
   {mode==='balance'&&balanceView!=='charts'&&<div className="balance-list-heading"><div><h3>Дэлгэрэнгүй жагсаалт</h3><p className="muted">{showCost?'Том тоо — ширхэг; доорх дүн — агуулахын өртөг (₮). ':''}Барааны нэрээс одоогийн мэдээллийг нээнэ.</p></div>{!grouped&&<SelectControl aria-label="Тайлан эрэмбэлэх" value={sort} onChange={e=>setFilter(setSort,e.target.value)}><option value="name">Нэрээр</option>{showCost&&<option value="value_desc">Өртөг ихээс</option>}<option value="stock_asc">Үлдэгдэл багаас</option><option value="out_desc">Зарлага ихээс</option></SelectControl>}</div>}
   <div className="inventory-commandbar"><div className="row inventory-actions">

    {showCost&&mode==='purchases'&&<Button disabled={!opts.warehouses.length} onClick={()=>openMovement('purchase')}><Plus size={16}/>Худалдан авалт</Button>}
   ...
   {mode==='balance'&&balanceView!=='charts'&&summary&&!list.loading&&!list.error&&!!(list.data?.groups?.length||rows.length)&&<InventoryBalanceTable rows={grouped?list.data?.groups||[]:rows} summary={summary} group={grouped} onOpen={setDetailId}/>}
   ...
   {['purchases','sales','moves'].includes(mode)&&(mode!=='balance'||balanceView!=='charts')&&!!rows.length&&!list.data?.groups?.length&&<div className="table-scroll"><Table><TableHeader><TableRow>
    ...
    {(mode==='items'||mode==='balance')&&<><TableCell><button className="inventory-item-link" onClick={()=>mode==='items'&&catalog?setProduct(r):setDetailId(r.id)}><strong>{r.name}</strong><small>{mode==='items'&&catalog?`${r.unit_count} дугаарын бүртгэл`:r.code}</small>{mode==='balance'&&<small>{r.category||'Ангилаагүй'} · {r.brand||'Брэнд бүртгээгүй'}</small>}</button></TableCell>{mode==='items'?<><TableCell>{r.category||'Ангилаагүй'}</TableCell><TableCell>{r.brand||'—'}</TableCell><TableCell>{r.supplier||'—'}</TableCell><TableCell>{[r.capacity,r.color,r.variant].filter(Boolean).join(' / ')||'—'}<small>{r.imei}{r.barcode?' · Баркод: '+r.barcode:''}</small></TableCell><TableCell><span className={r.stock<=r.min_stock?'inventory-low':''}>{r.stock.toLocaleString()} ш</span><small><StockTag item={r}/></small></TableCell>{showCost&&<TableCell>{cash(r.value_cents/100)}<small>Нэгж: {cash(r.stock?r.value_cents/r.stock/100:0)}</small></TableCell>}<TableCell>{cash(r.sale_price)}{r.sale_price_max!==undefined&&r.sale_price_max!==r.sale_price&&<small>— {cash(r.sale_price_max)}</small>}<small>Бэлэн: {cash(r.cash_price??r.sale_price)}{r.cash_price_max!==undefined&&r.cash_price_max!==(r.cash_price??r.sale_price)?' — '+cash(r.cash_price_max):''}</small></TableCell></>:<>{[[r.opening_qty,r.opening_cents],[r.in_qty,r.in_cents],[r.out_qty,r.out_cents],[r.stock,r.value_cents]].map(([qty,value],i)=><TableCell key={i}><strong>{qty.toLocaleString()} ш</strong>{showCost&&<small>{cash(value/100)}</small>}{i===3&&<StockTag item={r}/>}</TableCell>)}</>}</>}
    {mode==='purchases'&&<><TableCell>{dateLabel(r.received_at||r.created_at)}<small>{r.order_number}</small></TableCell><TableCell><button className="inventory-item-link" onClick={()=>setDetailId(r.item_id)}>{r.item_name}<small>{r.item_code}</small></button></TableCell><TableCell>{r.warehouse_name}</TableCell><TableCell>{r.qty} ш{r.returned_qty>0&&<small>Буцаасан {r.returned_qty}</small>}</TableCell>{showCost&&<TableCell>{cash(r.total_cost)}<small>Нэгж {cash(r.unit_cost)}</small></TableCell>}<TableCell>{purchaseStatuses[r.status]}<small>{r.payment_status}</small></TableCell><TableCell>{r.created_by==='user-approved:excel-purchase-history'?<Tag>Түүхэн импорт</Tag>:r.status==='ordered'?<Popconfirm title="Агуулахад хүлээн авах" description={<span>{r.item_name} · {r.item_code}<br/>{r.warehouse_name} · {r.qty} ш нэмэгдэнэ</span>} okText="Хүлээн авах" cancelText="Цуцлах" disabled={busy} onConfirm={async()=>{try{await post('receive_purchase',{},r.id);toast.success('Агуулахад хүлээн авлаа.');}catch(e){toast.error((e as Error).message);throw e;}}}><Button size="sm" disabled={busy}>Хүлээн авах</Button></Popconfirm>:r.returned_qty<r.qty&&<Button size="sm" variant="outline" onClick={()=>setModal({kind:'return',purchase:r})}>Буцаах</Button>}</TableCell></>}
    {mode==='sales'&&<><TableCell>{dateLabel(r.sold_at||r.created_at)}<small>{r.bill_number}</small></TableCell><TableCell><button className="inventory-item-link" onClick={()=>setDetailId(r.item_id)}>{r.item_name}<small>{r.item_code} · {r.warehouse_name}</small></button></TableCell><TableCell>{r.qty} ш</TableCell><TableCell>{cash(r.total_price)}{showCost&&<small>Өртөг {cash(r.cost_cents/100)}</small>}</TableCell><TableCell>{cash(r.commission_cents/100)}<small>Татвар {cash(r.tax_cents/100)}</small></TableCell>{showCost&&<TableCell className={r.profit_cents<0?'inventory-negative':''}>{cash(r.profit_cents/100)}</TableCell>}<TableCell>{r.platform||'—'}<small>{r.account}{r.vat_issued?' · Баримт олгосон':''}</small></TableCell><TableCell>{r.customer_name||'—'}<small>{r.customer_phone}</small></TableCell></>}
    {mode==='moves'&&<><TableCell>{dateLabel(r.occurred_at||r.created_at)}</TableCell><TableCell><button className="inventory-item-link" onClick={()=>setDetailId(r.item_id)}>{r.item_name}<small>{r.item_code}</small></button></TableCell><TableCell>{r.warehouse_name}</TableCell><TableCell>{kinds[r.kind]||r.kind}{r.ref_id&&r.kind.startsWith('transfer_')&&<small><button className="inventory-chart-link" onClick={()=>{query.set('ref',r.ref_id!);setMovementKind('');setWarehouse('');setPage(1);}}>Хоёр агуулахын хөдөлгөөн</button></small>}</TableCell><TableCell className={r.qty_delta<0?'inventory-negative':''}>{r.qty_delta>0?'+':''}{r.qty_delta}</TableCell>{showCost&&<TableCell>{cash(r.value_cents/100)}</TableCell>}<TableCell>{r.note||'—'}</TableCell></>}
   </TableRow>)}</TableBody>{mode==='balance'&&summary&&<TableFooter><TableRow><TableCell><strong>Нийт · {summary.count.toLocaleString()} бараа</strong><small>Бүх хуудасны дүн</small></TableCell>{[[summary.opening_qty,summary.opening_cents],[summary.in_qty,summary.in_cents],[summary.out_qty,summary.out_cents],[summary.units,summary.value_cents]].map(([qty,value],i)=><TableCell key={i}><strong>{qty.toLocaleString()} ш</strong>{showCost&&<small>{cash(value/100)}</small>}</TableCell>)}</TableRow></TableFooter>}</Table></div>}
   {mode!=='balance'&&!!list.data?.groups?.length&&<><p className="inventory-note">Ангилал нь барааны одоогийн ангилал, брэнд, нийлүүлэгчээр тооцогдоно. {mode==='items'?'Одоогийн үлдэгдэл.':'Сонгосон огноо, агуулахын хүрээнд.'}</p><div className="table-scroll"><Table><TableHeader><TableRow>{[grouped==='brand'?'БРЭНД':grouped==='category'?'АНГИЛАЛ':'НИЙЛҮҮЛЭГЧ','БАРААНЫ ТӨРӨЛ',mode==='sales'?'БОРЛУУЛСАН ТОО':'ҮЛДЭГДЭЛ','НИЙТ ӨРТӨГ',...(mode==='sales'?['БОРЛУУЛАЛТ','ШИМТГЭЛ / ТАТВАР','АШИГ']:[])].filter(h=>showCost||!costLabel(h)).map(h=><TableHead key={h}>{h}</TableHead>)}</TableRow></TableHeader><TableBody>{list.data.groups.map(g=><TableRow key={g.label}><TableCell><strong>{g.label||'Бүртгээгүй'}</strong>{!!g.cost_estimated&&<small>Өртөг ойролцоо</small>}</TableCell><TableCell>{g.item_count}</TableCell><TableCell>{g.stock.toLocaleString()} ш</TableCell>{showCost&&<TableCell>{cash(g.value_cents/100)}</TableCell>}{mode==='sales'&&<><TableCell>{cash(g.revenue_cents/100)}</TableCell><TableCell>{cash(Number(g.commission_cents)/100)}<small>{cash(Number(g.tax_cents)/100)}</small></TableCell>{showCost&&<TableCell>{cash(Number(g.profit_cents)/100)}</TableCell>}</>}</TableRow>)}</TableBody></Table></div></>}
 ...
 return <InventoryCostAccess.Provider value={showCost}><section className="table-panel inventory-panel">
  ...
  <Sheet open={!!detailId&&!modal} onOpenChange={o=>{if(!o)setDetailId('');}}><SheetContent className="detail-sheet inventory-detail"><SheetHeader><SheetTitle>{detail.data?.item.name||'Барааны дэлгэрэнгүй'}</SheetTitle><SheetDescription>Агуулах тус бүрийн үлдэгдэл, сүүлийн 100 хөдөлгөөн</SheetDescription></SheetHeader><div className="detail-body"><AsyncStatus error={detail.error} loading={detail.loading} retry={detail.retry}/>{detail.data&&<>
   ...
   ].filter(([label])=>showCost||!costLabel(String(label))).map(([label,value])=>({key:String(label),label,children:value||'—'}))}/>
   {!!detail.data.item.cost_estimated&&<p className="inventory-note">Өртгийн зарим дүнг нөхөн тооцсон тул ойролцоо утгатай.</p>}
   <div className="row inventory-actions">{canEditItem&&<Button size="sm" onClick={()=>setModal({kind:'item',item:detail.data!.item})}>Мэдээлэл засах</Button>}{showCost&&<Button size="sm" variant="outline" onClick={()=>openMovement('purchase',detail.data!.item)}>Орлого нэмэх</Button>}<Button size="sm" variant="outline" onClick={()=>openMovement('sale',detail.data!.item)}>Борлуулалт бүртгэх</Button><Button size="sm" variant="outline" disabled={opts.warehouses.length<2} onClick={()=>openMovement('transfer',detail.data!.item)}>Шилжүүлэх</Button></div>
   <h3>Агуулах тус бүрийн үлдэгдэл</h3><div className="sync-summary">{detail.data.byWarehouse.map(w=><div key={w.warehouse_id}><span>{w.warehouse_name}</span><strong>{w.qty} ш</strong>{showCost&&<small>{cash(w.value_cents/100)}</small>}</div>)}</div>
 ...
 </section></InventoryCostAccess.Provider>;
}
