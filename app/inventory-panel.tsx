...
export default function InventoryPanel({me,members}:{me:Member;members:Member[]}){
 ...
 const content=(mode==='counts'?<InventoryCountsPanel me={me} members={members}/>:<>
   ...
   <div className="inventory-filter-tags">{mode==='moves'&&query.get('ref')&&<Tag closable onClose={()=>query.set('ref','')}>Холбогдсон шилжүүлэг</Tag>}{[[warehouse,opts.warehouses.find(w=>w.id===warehouse)?.name||warehouse,setWarehouse],[brand,brand==='__unregistered__'?'Брэнд тодорхойгүй':brand,setBrand],[supplier,supplier==='__unregistered__'?'Нийлүүлэгч тодорхойгүй':supplier,setSupplier],[category,category==='__uncategorized__'?'Ангилаагүй':category,setCategory],...(['items','balance'].includes(mode)?[[stock,({nonzero:'0 үлдэгдлийг нуух',positive:'Үлдэгдэлтэй',empty:'Үлдэгдэлгүй',low:'Доод хэмжээнд',reorder:'Нөхөн татах'} as Record<string,string>)[stock],setStock]]:[])].filter(([value])=>value).map(([value,label,setter])=><Tag key={String(value)+String(label)} closable onClose={()=>setFilter(setter as (v:string)=>void,'')}>{String(label)}</Tag>)}</div>
