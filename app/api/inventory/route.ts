...
export async function GET(req:Request){try{
 ...
 if(view==='products'){
  ...
  if(productId){
   ...
   const website=await db().prepare('SELECT site_id,site_code,enabled,last_qty,last_synced_at,last_error FROM site_stock_links WHERE product_key=?').bind(productId).first();
   return json({product,items:units.results,count:count?.count||0,page,history:history.results,byWarehouse,website});
  }
