...
export async function GET(req:Request){try{const m=await member(),url=new URL(req.url),s=scope(m);const id=url.searchParams.get('id');if(id){const lead=await getLead(id,m);const activities=await db().prepare('SELECT * FROM activities WHERE lead_id=? ORDER BY created_at DESC LIMIT 100').bind(id).all();return Response.json({lead,activities:activities.results},{headers:{'Cache-Control':'no-store'}});}
 ...
 if(view==='duplicates'){
 if(m.role==='agent')throw new Failure('Зөвхөн удирдлага, админ харна.',403);
 const dpage=Math.max(1,Math.min(1000,Number(url.searchParams.get('page'))||1));
 const groupCount=await db().prepare('SELECT COUNT(*) n FROM (SELECT phone FROM leads GROUP BY phone HAVING COUNT(*)>1)').first<{n:number}>();
 const phones=await db().prepare('SELECT phone,COUNT(*) n FROM leads GROUP BY phone HAVING COUNT(*)>1 ORDER BY n DESC,phone LIMIT 20 OFFSET ?').bind((dpage-1)*20).all<{phone:string;n:number}>();
 const phoneList=phones.results.map(p=>p.phone);
 const leadRows=phoneList.length?await db().prepare(`SELECT id,name,phone,owner,status,created_at,source FROM leads WHERE phone IN (${phoneList.map(()=>'?').join(',')}) ORDER BY phone,created_at`).bind(...phoneList).all<{id:string;name:string;phone:string;owner:string;status:string;created_at:string;source:string}>():{results:[] as {id:string;name:string;phone:string;owner:string;status:string;created_at:string;source:string}[]};
 const groups=phones.results.map(p=>({phone:p.phone,count:p.n,leads:leadRows.results.filter(l=>l.phone===p.phone)}));
 return Response.json({groups,total:groupCount?.n||0,page:dpage},{headers:{'Cache-Control':'no-store'}});
 }
