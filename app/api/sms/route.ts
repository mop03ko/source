...
// Админ гараар дугаар оруулж чөлөөт мессеж илгээх зориулалттай; автомат (won) SMS-тэй адил sendSms ашиглана.
export async function POST(req:Request){try{
 if(req.headers.get('origin')!==new URL(req.url).origin)return Response.json({error:'Хүсэлт зөвшөөрөгдөхгүй.'},{status:403});
 const m=await member();if(m.role!=='admin')return Response.json({error:'Зөвхөн админ SMS илгээнэ.'},{status:403});
