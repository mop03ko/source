...
import {conversations,thread,send,markRead} from '@/lib/messages';
import {z} from 'zod';
...
export async function GET(req:Request){try{
 const m=await member(),url=new URL(req.url),peer=(url.searchParams.get('peer')||'').trim().toLowerCase();
...
export async function POST(req:Request){try{
 ...
 const b=z.discriminatedUnion('action',[z.object({action:z.literal('send'),peer:z.string().email(),body:z.string().trim().min(1).max(2000)}),z.object({action:z.literal('read'),peer:z.string().email()})]).parse(JSON.parse(text));
