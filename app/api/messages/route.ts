...
export async function GET(req:Request){try{
 ...
 if(url.searchParams.get('team')==='1'){const [items,reads]=await Promise.all([teamMessages(m.email),teamReadState(m.email)]);return Response.json({items,reads},{headers:{'Cache-Control':'no-store'}});}
...
export async function POST(req:Request){try{
 ...
 const b=z.discriminatedUnion('action',[
  ...
  z.object({action:z.literal('send_team'),body:z.string().trim().min(1).max(2000),replyTo:z.string().max(80).optional()}),
  z.object({action:z.literal('read_team')}),
 ...
 if(b.action==='send_team'){
  let replyTo=null;
  if(b.replyTo){replyTo=await messageSnapshot('team',b.replyTo,m.email);if(!replyTo)throw new Failure('Хариулах мессеж олдсонгүй.');}
  const r=await sendTeam(m.email,b.body,replyTo);return Response.json({ok:true,...r});
 }
 if(b.action==='read_team'){await markTeamRead(m.email);return Response.json({ok:true});}
 if(b.action==='react'){
  // Reaction зөвхөн харах эрхтэй мессежид л зөвшөөрнө (DM бол sender/recipient байх ёстой); messageSnapshot энэ шалгалтыг хийнэ.
  const snap=await messageSnapshot(b.kind,b.messageId,m.email);
 ...
 if(b.action==='send'){
  let replyTo=null;
  if(b.replyTo){replyTo=await messageSnapshot('dm',b.replyTo,m.email);if(!replyTo)throw new Failure('Хариулах мессеж олдсонгүй.');}
