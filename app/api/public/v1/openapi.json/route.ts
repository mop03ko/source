...
export function GET(){return Response.json({
 openapi:'3.1.0',info:{title:'AntMall Public Products API',version:'1.0.0',description:'Read-only grouped products and stock. No authentication. IMEI, barcode, supplier, cost and customer data are excluded. CDN responses may be up to 15 seconds old. Stock is not a checkout reservation.'},
 servers:[{url:'https://crm.antmall.mn'}],security:[],
 paths:{
  ...
  ],responses:{'200':{description:'Products ordered by ID',content:{'application/json':{schema:{type:'object',properties:{data:{type:'array',items:{$ref:'#/components/schemas/Product'}},pagination:{type:'object',properties:{page:{type:'integer'},limit:{type:'integer'},total:{type:'integer'},total_pages:{type:'integer'},has_more:{type:'boolean'}}},as_of:{type:'string',format:'date-time'}}}}}},'400':error,'503':error}}},
  '/api/public/v1/products/{id}':{get:{summary:'Get one group by ID or unified SKU',parameters:[{name:'id',in:'path',required:true,schema:{type:'string',maxLength:200}}],responses:{'200':{description:'Product group',content:{'application/json':{schema:{type:'object',properties:{data:{$ref:'#/components/schemas/Product'},as_of:{type:'string',format:'date-time'}}}}}},'400':error,'404':error,'503':error}}},
