import {previewAssignment} from '@/lib/assign';
...
const assignmentSchema=z.object({enabled:z.boolean(),automatic:z.boolean(),days:z.number().int().min(1).max(30),assignments:z.array(z.string().refine(v=>shiftAssignments.includes(v)&&!shiftOff.includes(v))).max(20),excluded_emails:z.array(z.string().email().transform(v=>v.toLowerCase())).max(100)}).strict();
...
export async function POST(req:Request){try{
 ...
 if(parsed?.preview_assignment!==undefined){const preview=z.object({preview_assignment:assignmentSchema}).strict().parse(parsed);return Response.json(await previewAssignment(preview.preview_assignment),{headers:{'Cache-Control':'no-store'}});}
 const b=z.object({notification_sound:z.enum(Object.keys(soundPresets) as [string,...string[]]).optional(),auto_assignment:assignmentSchema.optional()}).refine(v=>v.notification_sound||v.auto_assignment,'Тохиргоо буруу.').parse(parsed);
