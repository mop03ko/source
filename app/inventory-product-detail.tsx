...
export default function InventoryProductDetail({id,initialProduct,revision,warehouse,onClose,onOpenUnit,onAdd}:{id:string;initialProduct:Item;revision:number;warehouse:string;onClose:()=>void;onOpenUnit:(id:string)=>void;onAdd:(item:Item)=>void}){
 const [q,setQ]=useState(''),[page,setPage]=useState(1);
 const result=useRemote<ProductDetail>('/api/inventory?'+new URLSearchParams({view:'products',id,unit_q:q,page:String(page),warehouse_id:warehouse,revision:String(revision)}));
 const p=result.data?.product||initialProduct;
 return <Sheet open onOpenChange={o=>{if(!o)onClose();}}><SheetContent className="detail-sheet inventory-product-detail"><SheetHeader><SheetTitle>{p?.name||'Барааны бүртгэл'}</SheetTitle><SheetDescription>Ижил загвар, багтаамж, өнгөтэй бараа · Доторх дугаарын бүртгэлүүд</SheetDescription></SheetHeader><div className="detail-body"><AsyncStatus error={result.error} loading={result.loading} retry={result.retry}/>{p&&<>
  ...
  <div className="table-scroll"><Table><TableHeader><TableRow>{['ДУГААР','НИЙЛҮҮЛЭГЧ','ҮЛДЭГДЭЛ','ҮНЭ'].map(h=><TableHead key={h}>{h}</TableHead>)}</TableRow></TableHeader><TableBody>{result.data?.items.map(it=><TableRow key={it.id}><TableCell><button className="inventory-unit-link" onClick={()=>onOpenUnit(it.id)}><strong>{it.imei||it.barcode||it.code}</strong><small>IMEI/сериал: {it.imei||'—'}</small><small>Баркод: {it.barcode||'—'}</small><small>Код: {it.code||'—'}</small></button></TableCell><TableCell>{it.supplier||'Бүртгээгүй'}</TableCell><TableCell><Tag color={it.stock>0?'green':'default'}>{it.stock} ш</Tag></TableCell><TableCell>{cash(it.sale_price)}<small>Бэлэн: {cash(it.cash_price??it.sale_price)}</small></TableCell></TableRow>)}</TableBody></Table></div>
  {!result.data?.items.length&&<p className="muted">Тохирох дугаар олдсонгүй.</p>}<Pagination current={page} total={result.data?.count||0} pageSize={50} showSizeChanger={false} onChange={setPage} hideOnSinglePage/>
