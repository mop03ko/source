...
export function MovementForm({kind,item,options,warehouse,busy,onSave}:{kind:'purchase'|'sale'|'transfer';item?:Item;options:Options;warehouse:string;busy:boolean;onSave:(data:unknown)=>Promise<void>}){
 ...
 return <GuardedForm className="form-stack" onSubmit={async e=>{
  ...
  <Field label={kind==='purchase'?'Хүлээн авах агуулах *':'Зарлагадах агуулах *'}><select name="warehouse_id" value={source} onChange={e=>setSource(e.target.value)} required><option value="">Сонгох…</option>{options.warehouses.map(w=><option value={w.id} key={w.id}>{w.name}</option>)}</select></Field>
  ...
  {kind==='transfer'&&<Field label="Очих агуулах *"><select name="to_warehouse_id" required defaultValue=""><option value="">Сонгох…</option>{options.warehouses.filter(w=>w.id!==source).map(w=><option key={w.id} value={w.id}>{w.name}</option>)}</select></Field>}
  ...
  {kind==='purchase'&&<>
   <div className="form-grid"><Field label="Захиалгын дугаар"><Input name="order_number" maxLength={120}/></Field><Field label="Төлөв"><select name="status" value={purchaseStatus} onChange={e=>setPurchaseStatus(e.target.value)}><option value="received">Хүлээн авсан</option><option value="ordered">Захиалсан · хүлээгдэж буй</option></select></Field></div>
   ...
   <div className="form-grid"><Field label="Нийт нэмэлт зардал (тээвэр, бэлтгэл)"><Input name="additional_cost" type="number" min={0} step="0.01" value={extra} onChange={e=>setExtra(Number(e.target.value))}/></Field><Field label="Төлбөр"><select name="payment_status"><option>Төлбөр төлөгдсөн</option><option>Дараа тооцоо</option><option>Хэсэгчлэн төлсөн</option></select></Field></div>
