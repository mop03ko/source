...
export default function ItPanel({me,members,initialTaskId}:{me:Member;members:Member[];initialTaskId?:string}){
 ...
 <Timeline items={detail.activities.map(a=>({key:a.id,content:<><div className="row between"><strong>{ownerName(a.actor)}</strong><time>{dateLabel(a.created_at)}</time></div><p>{a.note}</p></>}))}/>
